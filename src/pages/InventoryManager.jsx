...
function isPendingInventoryDemand(order) {
...
function buildPendingDemand(orders) {
  const demand =
    new Map();
...
function InventoryManager({ onNavigate = () => {} }) {
  ...
  async function loadCatalog(secret = adminSecret) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          "The protected order service returned an invalid response."
        );
      }
      ...
      if (
        ...
        throw new Error(
          orderResult.error ||
            "Pending order demand could not be loaded."
        );
      }
  ...
  }, [allVariants, pendingDemandByCode, products.length]);
    ...
    return (
      <main className="catalog-admin-page">
        ...
        <section className="catalog-lock-card">
          ...
          <p>
            Enter the same administrator secret used by Orders, Customer Accounts,
            and Product Manager.
          </p>
      ...
      <section className="catalog-admin-inner">
        <header className="catalog-admin-hero">
          <div>
            ...
            <h1>Inventory Manager</h1>
            <p>
              Manage on-hand quantities, low-stock warnings, storefront status,
              and pending unpaid-order demand. Inventory is deducted when payment
              is recorded and restored when an eligible order is cancelled.
            </p>
          </div>
        ...
        <section className="catalog-summary-grid">
          ...
          <SummaryCard label="Units On Hand" value={summary.totalUnits} />
          <SummaryCard label="Pending Demand" value={summary.pendingUnits} />
          <SummaryCard label="Available After Pending" value={summary.projectedAvailable} />
          <SummaryCard label="Inventory Cost" value={formatMoney(summary.inventoryCost)} />
          <SummaryCard label="Low / Empty" value={summary.lowStock} warning />
          <SummaryCard label="Preorders" value={summary.preorder} />
        </section>

        <section className="catalog-inventory-note">
          <strong>
            Pending demand is advisory.
          </strong>

          <span>
            New and invoiced orders count against projected availability until
            payment is recorded. They do not reduce storefront stock automatically.
          </span>
        </section>
                        ...
                        {(product.variants || []).map((variant) => {
                          ...
                          >
                            ...
                            <div className="catalog-variant-details">
                              ...
                              <span>
                                <small>Pending Demand</small>
                                <strong>
                                  {pending}
                                </strong>
                              </span>

                              <span className={projected < 0 ? "danger" : ""}>
                                <small>Available After Pending</small>
                                <strong>
                                  {variant.trackQuantity
...
.catalog-summary-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px; margin: 18px 0; }
.catalog-summary-card { min-width: 0; min-height: 92px; padding: 18px; border-radius: 18px; border: 1px solid rgba(255,255,255,.09); background: #11151b; display: grid; align-content: center; gap: 8px; overflow: hidden; }
.catalog-summary-card span { min-width: 0; color: #99a7b8; font-size: 12px; line-height: 1.35; text-transform: uppercase; letter-spacing: .06em; font-weight: 800; overflow-wrap: anywhere; word-break: normal; }
.catalog-summary-card strong { min-width: 0; font-size: 28px; line-height: 1.2; overflow-wrap: anywhere; }
...
.catalog-pill { display: inline-flex; align-items: center; justify-content: center; max-width: 100%; border-radius: 999px; padding: 7px 10px; font-size: 11px; line-height: 1.25; text-align: center; font-weight: 900; white-space: normal; overflow-wrap: anywhere; }
...
.catalog-variant-details { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 8px; margin: 15px 0; }
.catalog-variant-details span { min-width: 0; border-radius: 12px; background: rgba(255,255,255,.04); padding: 10px; overflow: hidden; }
.catalog-variant-details small, .catalog-variant-details strong { display: block; min-width: 0; overflow-wrap: anywhere; }
.catalog-variant-details small { color: #8290a0; margin-bottom: 4px; }
.catalog-variant-details .danger { border: 1px solid rgba(255,83,83,.3); background: rgba(255,83,83,.08); }
.catalog-inventory-note { display: grid; gap: 5px; padding: 14px 16px; margin-bottom: 14px; border: 1px solid rgba(61,165,255,.28); border-radius: 14px; background: rgba(61,165,255,.07); }
.catalog-inventory-note span { color: #b7c6d5; line-height: 1.55; }
